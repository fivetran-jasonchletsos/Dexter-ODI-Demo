...
export default function Hero() {
  const cs = codeStats();
  return (
    <section className="relative overflow-hidden border-b border-slide/20 px-5 py-16 sm:px-6 sm:py-20 md:px-16 md:py-24">
      <div className="blood-spatter absolute inset-0 pointer-events-none" aria-hidden="true" />
      <div className="forensic-reticle" aria-hidden="true" />
      <div className="relative mx-auto max-w-6xl">
        <p className="reveal reveal-1 type text-[10px] uppercase tracking-[0.35em] text-signal mb-4 slide-mark">
          Evidence Locker · Archivist: J. Chletsos
        </p>
        <h1 className="reveal reveal-2 serif text-6xl sm:text-7xl md:text-8xl leading-[0.92] text-bone">
          <span className="slide-underline">Every kill.</span> Every season.<br className="hidden sm:block" /> Every blood slide.
        </h1>
        <p className="reveal reveal-3 serif mt-7 max-w-2xl text-xl text-ash/90 leading-relaxed">
          The full Dexter Morgan case file. Filter it, rank it, find out which kills broke the Code.
        </p>

        <div className="reveal reveal-4 mt-10 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl">
        ...
        <div className="reveal reveal-5 mt-9 flex flex-wrap gap-3">
...
function BigStat({ value, label, tint }: { value: number | string; label: string; tint?: "slide" | "neon" }) {
  const color =
    ...
    tint === "neon"  ? "text-neon"   :
                       "text-bone";
  const accent =
    tint === "slide" ? "border-l-signal/60" :
    tint === "neon"  ? "border-l-neon/50"   :
                       "border-l-bone/20";
  return (
    <div className={`evidence-card rounded-sm p-4 border-l-2 ${accent}`}>
      <p className={`serif text-5xl sm:text-6xl leading-none ${color}`}>{value}</p>
      <p className="type text-[10px] uppercase tracking-[0.25em] text-ash/50 mt-3">{label}</p>
    </div>
  );
}
